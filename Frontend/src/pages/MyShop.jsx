...
import ShopLocationPicker from '../components/delivery/ShopLocationPicker';
...
export default function MyShop(){
  ...
  useEffect(()=>{
    ...
    if(user){
      ...
      }).catch(()=>setError(t('myShop.errorLoadingShops')));
    }
  },[user, dispatch]);

  const selectedShop = shops.find(s=>s.id===selectedShopId) || null;

  if(!user) return <div className='p-6'>{t('myShop.pleaseLogin')}</div>;
  if(user.role !== 'shop_owner') return <div className='p-6'>{t('myShop.onlyShopOwner')}</div>;
  ...
  const handleDelete = (id) => {
    if(!id) return;
  if(window.confirm(t('myShop.confirmDelete'))){
  ...
  <h1 className='text-3xl font-bold bg-gradient-to-r from-emerald-600 to-slate-800 bg-clip-text text-transparent tracking-tight'>{t('myShop.title')}</h1>
      <div className='flex items-center gap-3 text-xs text-slate-500'>
  <span className='px-2 py-1 rounded-lg bg-white/70 border border-slate-200 backdrop-blur-sm'>{t('myShop.mode')}: <span className='font-semibold text-slate-700'>{mode}</span></span>
        {!!shops.length && mode==='view' && (
          <button onClick={openCreate} className='px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium shadow-sm hover:bg-emerald-700 transition'>+ {t('myShop.newShop')}</button>
        )}
        {mode!=='view' && (
          <button onClick={cancel} className='px-4 py-2 rounded-lg bg-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-300 transition'>{t('myShop.back')}</button>
        )}
      </div>
    </div>
  {loading && <p className='text-sm text-slate-500 mb-4'>{t('myShop.loading')}</p>}
    ...
    <div className='grid xl:grid-cols-[300px_1fr] gap-10 items-start'>
      ...
      <aside className='space-y-5 xl:sticky xl:top-6 self-start'>
        <div className='flex items-center justify-between'>
          <h2 className='text-lg font-semibold text-slate-800'>{t('myShop.myShops')}</h2>
          <button onClick={openCreate} className='px-2.5 py-1.5 rounded-md bg-emerald-600 text-white text-xs font-medium hover:bg-emerald-700'>+ {t('myShop.new')}</button>
        </div>
              ...
              <div key={s.id} onClick={()=>setSelectedShopId(s.id)} className={`group relative p-4 rounded-xl border cursor-pointer transition shadow-sm ${active ? 'border-emerald-500 ring-2 ring-emerald-200 bg-white' : 'border-slate-200 bg-white/60 hover:bg-white hover:border-emerald-300'}`}> 
                <div className='flex justify-between items-start gap-2'>
                  <div className='min-w-0'>
                    ...
                    <p className='text-[11px] text-slate-500 truncate'>{s.city || t('myShop.noValue')}</p>
                  </div>
                  <div className='flex gap-1 opacity-0 group-hover:opacity-100 transition'>
                    <button onClick={(e)=>{e.stopPropagation(); startEdit(s);}} className='px-2 py-1 text-[10px] rounded bg-slate-100 hover:bg-slate-200 text-slate-600'>{t('myShop.edit')}</button>
                    <button onClick={(e)=>{e.stopPropagation(); handleDelete(s.id);}} className='px-2 py-1 text-[10px] rounded bg-red-100 hover:bg-red-200 text-red-600'>{t('myShop.delete')}</button>
                  </div>
                </div>
              </div>
            );
          })}
          {!shops.length && <p className='text-xs text-slate-500'>{t('myShop.noShop')}</p>}
        </div>
      </aside>
      ...
      <section className='space-y-8'>
          ...
          <div className='bg-white/80 backdrop-blur-sm p-7 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition group'>
            <div className='flex flex-wrap gap-4 justify-between items-start mb-4'>
              <div className='space-y-1 min-w-[220px]'>
                ...
                <p className='text-xs text-slate-500'>{t('myShop.lastUpdate')}: {selectedShop.updated_at ? new Date(selectedShop.updated_at).toLocaleDateString() : t('myShop.noValue')}</p>
              </div>
              <div className='flex flex-wrap gap-2'>
                <button onClick={()=>startEdit(selectedShop)} className='px-4 py-2 rounded-lg text-sm font-medium bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm'>{t('myShop.edit')}</button>
                <button onClick={()=>handleDelete(selectedShop.id)} disabled={deleting} className='px-4 py-2 rounded-lg text-sm font-medium bg-red-600 hover:bg-red-700 text-white disabled:opacity-50 shadow-sm'>{deleting ? t('myShop.deleting') : t('myShop.delete')}</button>
              </div>
            </div>
            <div className='grid md:grid-cols-3 gap-6'>
              <div className='md:col-span-2 space-y-4'>
                <p className='text-slate-600 text-sm leading-relaxed'>{selectedShop.description || <span className='italic text-slate-400'>{t('myShop.noDescription')}</span>}</p>
                <div className='grid sm:grid-cols-2 gap-4 text-sm'>
                  <div className='p-3 rounded-xl bg-slate-50 border border-slate-100'>
                    <p className='text-[11px] uppercase tracking-wide text-slate-500 font-medium'>{t('myShop.address')}</p>
                    <p className='text-slate-700 font-medium'>{selectedShop.address || t('myShop.noValue')}</p>
                  </div>
                  <div className='p-3 rounded-xl bg-slate-50 border border-slate-100'>
                    <p className='text-[11px] uppercase tracking-wide text-slate-500 font-medium'>{t('myShop.city')}</p>
                    <p className='text-slate-700 font-medium'>{selectedShop.city || t('myShop.noValue')}</p>
                  </div>
                  {(selectedShop.latitude && selectedShop.longitude) && (
                    <div className='p-3 rounded-xl bg-slate-50 border border-slate-100 sm:col-span-2'>
                      <p className='text-[11px] uppercase tracking-wide text-slate-500 font-medium'>{t('myShop.coordinates')}</p>
              ...
              <div className='space-y-3'>
                <div className='p-4 rounded-xl bg-gradient-to-br from-emerald-50 to-white border border-emerald-100'>
                  <p className='text-[11px] uppercase tracking-wide text-emerald-600 font-semibold'>{t('myShop.stats')}</p>
                  <div className='mt-2 flex flex-col gap-2'>
                    <div className='flex items-center justify-between text-sm'>
                      <span className='text-slate-600'>{t('myShop.products')}</span>
                      <span className='font-semibold text-slate-800'>—</span>
                    </div>
                    <div className='flex items-center justify-between text-sm'>
                      <span className='text-slate-600'>{t('myShop.createdAt')}</span>
                      <span className='font-semibold text-slate-800'>{selectedShop.created_at ? new Date(selectedShop.created_at).toLocaleDateString() : t('myShop.noValue')}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className='bg-white/80 backdrop-blur-sm p-7 rounded-2xl border border-slate-200 shadow-sm'>
          <div className='flex items-center justify-between mb-5'>
            <h3 className='text-xl font-semibold text-slate-800 tracking-tight'>{t('myShop.productManagement')}</h3>
            {!selectedShop && <span className='text-xs text-slate-400'>{t('myShop.selectShopToManageProducts')}</span>}
          </div>
        ...
        <div className='grid md:grid-cols-2 gap-8 items-start'>
          <form onSubmit={submitEdit} className='space-y-5 bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-white/30 order-1'>
            <div className='flex justify-between items-center'>
              <h2 className='text-xl font-semibold text-slate-800'>{t('myShop.editMyShop')}</h2>
              <button type='button' onClick={cancel} className='text-sm text-slate-500 hover:text-slate-700'>{t('myShop.cancel')}</button>
            </div>
            <div>
              <label className='block text-sm font-semibold mb-1 text-slate-700'>{t('myShop.name')}</label>
            ...
            <div>
              <label className='block text-sm font-semibold mb-1 text-slate-700'>{t('myShop.description')}</label>
            ...
            <div>
              <label className='block text-sm font-semibold mb-1 text-slate-700'>{t('myShop.address')}</label>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                <label className='block text-sm font-semibold mb-1 text-slate-700'>{t('myShop.city')}</label>
              ...
              <div className='flex items-end'>
                <div className='text-xs text-slate-500'>{t('myShop.updateViaMap')}</div>
              </div>
            </div>
            <div className='flex gap-3'>
              <button disabled={updating} className='bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white px-6 py-3 rounded-xl font-semibold shadow-lg shadow-emerald-300/40 disabled:opacity-50'>
                {updating ? t('myShop.saving') : t('myShop.save')}
              </button>
              <button type='button' onClick={cancel} className='px-6 py-3 rounded-xl font-semibold border border-slate-300 text-slate-600 hover:bg-slate-100'>{t('myShop.cancel')}</button>
            </div>
          </form>
          <div className='space-y-4 order-2'>
            <div className='bg-white/80 backdrop-blur-sm p-4 rounded-2xl shadow border border-white/30'>
              <label className='block text-sm font-semibold mb-2 text-slate-700'>{t('myShop.location')}</label>
              <ShopLocationPicker value={selectedLocation} onChange={loc=> setSelectedLocation(loc)} />
              <p className='mt-3 text-xs text-slate-500'>{t('myShop.mapTipEdit')}</p>
            </div>
          </div>
        </div>
      )}
  ...
  { (mode==='create' || !shops.length) && (
        <div className='grid xl:grid-cols-[1fr_420px] gap-10 items-start'>
          <form onSubmit={submit} className='space-y-5 bg-white/90 backdrop-blur-sm p-8 rounded-2xl shadow-lg border border-slate-200 order-1'>
            ...
            <div>
              <label className='block text-sm font-semibold mb-1 text-slate-700'>{t('myShop.addressOptional')}</label>
              <input className='border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 rounded-xl w-full p-3 bg-white text-slate-800 placeholder-slate-400' value={form.address} onChange={e=>setForm(f=>({...f,address:e.target.value}))} />
            </div>
            <div className='grid grid-cols-2 gap-4'>
              <div>
              ...
              <div className='flex items-end'>
                <div className='text-xs text-slate-500'>{t('myShop.mapTipCreate')}</div>
              </div>
            </div>
            <div className='flex justify-start items-center gap-3'>
              <button disabled={creating} className='bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white px-6 py-3 rounded-xl font-semibold shadow-lg shadow-emerald-300/40 disabled:opacity-50'>
                {creating ? t('myShop.creating') : t('myShop.createShop')}
              </button>
              {shops.length > 0 && (
                <button type='button' onClick={cancel} className='px-6 py-3 rounded-xl font-semibold border border-slate-300 text-slate-600 hover:bg-slate-100'>{t('myShop.cancel')}</button>
              )}
          ...
          <div className='space-y-4 order-2'>
            <div className='bg-white/90 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-slate-200'>
              <label className='block text-sm font-semibold mb-2 text-slate-700'>{t('myShop.location')}</label>
              <ShopLocationPicker value={selectedLocation} onChange={loc=> setSelectedLocation(loc)} />
              <p className='mt-3 text-xs text-slate-500'>{t('myShop.mapTipCreate2')}</p>
            </div>
          </div>
        </div>
      )}
