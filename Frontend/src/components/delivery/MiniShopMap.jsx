...
export default function MiniShopMap({ latitude, longitude, name }) {
  ...
  if (!latitude || !longitude) {
    return <div className="w-full h-48 flex items-center justify-center text-xs text-slate-500 bg-slate-100 rounded-xl">{t('delivery.locationNotProvided', 'Localisation non fournie')}</div>;
  }
  if (!token) {
    return <div className="w-full h-48 flex flex-col items-center justify-center text-xs text-slate-600 bg-slate-100 rounded-xl">
      <span>{t('delivery.mapboxTokenMissing', 'Mapbox token manquant')}</span>
      ...
      {!loaded && !error && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/70 backdrop-blur-sm">
          <div className="text-center">
            ...
            <p className="text-[11px] text-slate-600">{t('delivery.loading', 'Chargement...')}</p>
          </div>
        </div>
      )}
      {error && (
