...
const DeliveryTracking = ({ trackingNumber, orderId = null }) => {
  ...
  setError(data.error || t('delivery.trackingNotFound'));
      }
    } catch (err) {
      console.error('Tracking error:', err);
  setError(t('delivery.trackingConnectionError'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
      ...
      {loading && (
        <div className="text-center py-8">
          ...
          <p className="text-slate-600">{t('delivery.trackingSearching')}</p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-emerald-50 to-green-50 p-6 rounded-xl border border-emerald-200">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-emerald-600">
                  {t('delivery.order')} #{trackingInfo.order_id || orderId}
                </p>
              </div>
            ...
            <p className="text-sm text-emerald-600 text-center">
              {getProgressPercentage(trackingInfo.status)}% {t('delivery.completed')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-purple-50 p-4 rounded-xl border border-purple-200">
              ...
              <div className="text-sm text-purple-700 space-y-1">
                <p><strong>{t('delivery.fee')}:</strong> {trackingInfo.delivery_fee} TND</p>
                <p><strong>{t('delivery.distance')}:</strong> {trackingInfo.distance_km} km</p>
                <p><strong>{t('delivery.estimatedTime')}:</strong> {trackingInfo.estimated_delivery_time}</p>
                {trackingInfo.delivery_date && (
                  <p><strong>{t('delivery.estimatedDate')}:</strong> {new Date(trackingInfo.delivery_date).toLocaleDateString('fr-FR')}</p>
                )}
              </div>
            </div>
          </div>
                  ...
                  <div key={index} className="flex items-start space-x-4 pb-3 border-b border-slate-200 last:border-b-0 last:pb-0">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <h6 className="font-medium text-slate-800">{getStatusText(event.status)}</h6>
