...
const DeliveryZones = ({ selectedGovernorate }) => {
    ...
    return (
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
        <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center">
          🚚 {t('delivery.zonesTitle', 'Zones de livraison')}
        </h3>
        <div className="text-center py-8">
          <div className="text-slate-400 text-4xl mb-4">📦</div>
          <p className="text-slate-600">{t('delivery.selectGovernorate', 'Sélectionnez un gouvernorat pour voir les options de livraison')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
      <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center">
        🚚 {t('delivery.infoTitle', 'Information de livraison')}
      </h3>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">{t('delivery.selectedGovernorate', 'Gouvernorat sélectionné')}</p>
            </div>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-xl border-2 ${getColorClasses(zoneInfo.color)}`}>
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-bold">{t('delivery.zone', { zone: zoneInfo.zone, defaultValue: 'Zone {{zone}}' })}</h4>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              ...
              <div className="text-sm opacity-75">{t('delivery.fee', 'Frais de livraison')}</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{zoneInfo.time}</div>
              <div className="text-sm opacity-75">{t('delivery.estimatedTime', 'Délai estimé')}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-4">
          <h5 className="font-medium text-slate-800 mb-3">{t('delivery.legendTitle', 'Légende des zones')}</h5>
          <div className="space-y-2 text-sm">
            <div className="flex items-center">
              <div className="w-4 h-4 bg-emerald-500 rounded-full mr-3"></div>
              <span>{t('delivery.legendZone1', 'Zone 1: Grand Tunis (5 DT - 24-48h)')}</span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 bg-blue-500 rounded-full mr-3"></div>
              <span>{t('delivery.legendZone2', 'Zone 2: Nord/Centre (6-8 DT - 48-72h)')}</span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 bg-orange-500 rounded-full mr-3"></div>
              <span>{t('delivery.legendZone3', 'Zone 3: Sud/Intérieur (8-11 DT - 72-96h)')}</span>
            </div>
          </div>
        </div>
      </div>
