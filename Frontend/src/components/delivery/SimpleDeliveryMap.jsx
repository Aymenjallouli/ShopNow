    ...
    <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 overflow-hidden">
      <div className="p-4 border-b border-white/20">
        <h3 className="text-xl font-bold text-slate-800 flex items-center">
          🗺️ {t('delivery.addressSelection', "Sélection d'adresse")}
        </h3>
      ...
      <div className="relative h-64 bg-gradient-to-br from-emerald-50 to-blue-50 flex items-center justify-center overflow-hidden">
        ...
        <div className="text-center z-5">
          ...
          <h3 className="text-xl font-bold text-slate-800 mb-2">
            {customLocation ? t('delivery.ourLocation', 'Notre Localisation') : t('delivery.mapTitle', 'Carte de livraison')}
          </h3>
          <p className="text-slate-600 max-w-sm">
            {customLocation ? 
              t('delivery.findUsAt', { address: customLocation.address, defaultValue: 'Vous pouvez nous trouver à {{address}}' }) :
              (shippingInfo && shippingInfo.address ? 
                t('delivery.shippingTo', { city: shippingInfo.city, country: shippingInfo.country, defaultValue: 'Livraison vers {{city}}, {{country}}' }) : 
                t('delivery.fillFormToSeeLocation', 'Remplissez le formulaire de livraison pour voir votre localisation'))
            }
          </p>
          {customLocation ? (
        ...
        <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm rounded-lg p-3 shadow-lg border border-white/20">
          <div className="space-y-2 text-xs">
            <div className="flex items-center">
              ...
              <span className="text-slate-700">{t('delivery.yourAddress', 'Votre adresse')}</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-emerald-500 rounded-full mr-2"></div>
              <span className="text-slate-700">{t('delivery.deliveryZones', 'Zones de livraison')}</span>
            </div>
            <div className="flex items-center">
