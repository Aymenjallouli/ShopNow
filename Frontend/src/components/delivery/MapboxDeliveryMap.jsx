    ...
    return (
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
        <div className="text-center py-8">
          ...
          <p className="text-slate-600">{t('delivery.mapboxTokenMissing', "Le token Mapbox n'est pas configuré.")}</p>
        </div>
      </div>
    );
  }
      ...
      {showAddressSearch && false && (
        <div className="p-4 border-b border-white/20">
          <div className="relative">
            <input
              type="text"
              placeholder={t('delivery.searchPlaceholder', 'Rechercher une adresse en Tunisie...')}
              value={searchAddress}
              onChange={(e) => {
        ...
        {mapError ? (
          <div className="h-96 w-full flex items-center justify-center bg-slate-100 rounded-lg">
            <div className="text-center">
              <div className="text-red-500 text-4xl mb-4">⚠️</div>
              <h3 className="text-lg font-bold text-slate-800 mb-2">{t('delivery.mapError', 'Erreur de carte')}</h3>
            ...
            {!mapLoaded && (
              <div className="absolute inset-0 flex items-center justify-center bg-slate-100 rounded-lg">
                <div className="text-center">
                  ...
                  <p className="text-slate-600">{t('delivery.loadingMap', 'Chargement de la carte...')}</p>
                </div>
              </div>
            )}
          </div>
