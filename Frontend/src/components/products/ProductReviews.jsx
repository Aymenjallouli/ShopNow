    ...
    <div className="bg-slate-50 rounded-lg p-5 mb-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div className="flex flex-col items-center mb-4 md:mb-0">
          ...
          <p className="text-sm text-slate-500">{t('productReviews.reviewCount', { count: totalReviews }, { defaultValue: '{{count}} avis' })}</p>
        </div>
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-slate-200 mt-12">
      <div className="px-6 py-5 border-b border-slate-200 flex flex-wrap items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-800">{t('productReviews.title', 'Avis clients')}</h2>
        {!hasUserReviewed && isAuthenticated && (
          ...
          >
            ...
            <span>{showForm ? t('productReviews.cancel', 'Annuler') : t('productReviews.writeReview', 'Écrire un avis')}</span>
          </button>
        )}
        {hasUserReviewed && (
          <div className="flex space-x-2">
            ...
            >
              ...
              <span>{t('productReviews.edit', 'Modifier')}</span>
            </button>
            <button
              onClick={(e) => {
                e.stopPropagation();
            ...
            >
              ...
              <span>{confirmDelete === userReview.id ? t('productReviews.confirm', 'Confirmer') : t('productReviews.delete', 'Supprimer')}</span>
            </button>
          </div>
        )}
      </div>
          ...
          <div>
            {showForm ? (
              ...
              <div className="text-center py-12 px-4 bg-slate-50 rounded-lg">
                ...
                <p className="text-slate-600 mb-6">{t('productReviews.noReviewsYet', "Il n'y a pas encore d'avis pour ce produit.")}</p>
                {isAuthenticated ? (
                  <button
                    onClick={toggleReviewForm}
                  ...
                  >
                    ...
                    <span>{t('productReviews.beFirst', 'Soyez le premier à donner votre avis')}</span>
                  </button>
                ) : (
                  <a
                  ...
                  >
                    ...
                    <span>{t('productReviews.loginToReview', 'Connectez-vous pour laisser un avis')}</span>
                  </a>
                )}
              </div>
            )}
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center justify-between pb-6 border-b border-slate-200 mb-6">
              <div className="flex flex-wrap items-center space-x-2 mb-3 sm:mb-0">
                <span className="text-sm font-medium text-slate-700">{t('productReviews.filter', 'Filtrer :')}</span>
                <div className="flex space-x-1">
                  ...
                  {filterRating > 0 && (
                    ...
                    >
                      ...
                      {t('productReviews.showAll', 'Tout voir')}
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center">
                <span className="text-sm font-medium text-slate-700 mr-2">{t('productReviews.sortBy', 'Trier par :')}</span>
                <select
                  value={sortBy}
                ...
                >
                  <option value="recent">{t('productReviews.sortRecent', 'Plus récents')}</option>
                  <option value="highest">{t('productReviews.sortHighest', 'Meilleures notes')}</option>
                  <option value="lowest">{t('productReviews.sortLowest', 'Notes les plus basses')}</option>
                </select>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {filteredReviews.length === 0 ? (
                <div className="text-center py-10 bg-slate-50 rounded-lg">
                  <p className="text-slate-600">{t('productReviews.noReviewsFilter', 'Aucun avis ne correspond à votre filtre.')}</p>
                </div>
              ) : (
                  ...
                  <div key={review.id} className="border-b border-slate-200 pb-6 last:border-b-0 last:pb-0">
                    <div className="flex flex-wrap items-start justify-between mb-2">
                      <div className="flex flex-col">
                        <h4 className="text-lg font-semibold text-slate-900 mb-1">{review.title || t('productReviews.review', 'Avis')}</h4>
                        <div className="flex items-center">
                          <div className="flex items-center">
                          ...
                          <p className="ml-2 text-sm text-slate-500">
                            {t('productReviews.ratingOutOf', { rating: review.rating }, { defaultValue: '{{rating}} sur 5 étoiles' })}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center mb-3">
                      ...
                      <h5 className="text-sm font-medium text-slate-700">
                        {review.user?.name || t('productReviews.anonymous', 'Utilisateur anonyme')}
                      </h5>
                      {review.user?.id === user?.id && (
                        <span className="ml-2 text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">
                          {t('productReviews.you', 'Vous')}
                        </span>
                      )}
                    </div>
