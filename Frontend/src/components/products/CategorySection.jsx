...
const CategorySection = ({ categories, onCategorySelect, selectedCategory }) => {
  ...
  const defaultCategoryImage = '/placeholder.svg';
    ...
    <div className="mb-12">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent mb-2">
            {t('categorySection.title', 'Browse Categories')}
          </h2>
          <p className="text-slate-600">
            {t('categorySection.subtitle', 'Find your favorite products by category')}
          </p>
        </div>
      </div>
      ...
      {safeCategories.length === 0 ? (
        <div className="flex justify-center items-center p-8 bg-slate-50 rounded-xl">
          <p className="text-slate-500">{t('categorySection.noCategories', 'No categories available')}</p>
        </div>
      ) : (
                ...
                <div className="block h-full">
                  <div className="aspect-w-16 aspect-h-9 w-full relative">
                    <img
                      ...
                      alt={category?.name || t('categorySection.categoryAlt', 'Category')}
                  ...
                  <div className="p-4">
                    ...
                    `}>
                      {category?.name || t('categorySection.unknownCategory', 'Unknown Category')}
                    </h3>
                  </div>
                </div>
              </div>
