  ...
  return (
    <div className='space-y-4'>
      <div className='relative group'>
        <label className='text-xs font-medium text-slate-500 mb-1 block tracking-wide'>Recherche d'adresse</label>
        <div className='flex items-center gap-2'>
          ...
          {query && (
            <button type='button' onClick={()=>{setQuery(''); setResults([]);}} className='text-xs px-3 py-2 rounded-lg bg-slate-200 hover:bg-slate-300 text-slate-700'>Effacer</button>
          )}
        </div>
        {results.length > 0 && (
        ...
        {loadingMap && (
          <div className='absolute inset-0 flex items-center justify-center bg-white/70 backdrop-blur-sm'>
            <div className='text-center'>
              ...
              <p className='text-xs text-slate-600'>Chargement de la carte...</p>
            </div>
          </div>
        )}
      </div>
        ...
        <div className='text-xs text-slate-600 flex-1'>
          {value ? (
            <div className='space-y-1'>
              <p className='font-medium text-slate-700'>Adresse sélectionnée:</p>
              <p className='truncate text-slate-700'>{value.address}</p>
              <div className='flex gap-4 text-slate-500'>
                <span>Lat: {value.coordinates.lat.toFixed(5)}</span>
                <span>Lng: {value.coordinates.lng.toFixed(5)}</span>
              </div>
            </div>
          ) : 'Cliquez sur la carte ou utilisez la recherche pour sélectionner la localisation.'}
        </div>
        {value && <button type='button' onClick={clearLocation} className='text-xs px-3 py-2 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 border border-red-200'>Retirer</button>}
      </div>
    </div>
  );
};
