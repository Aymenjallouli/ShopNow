  ...
  return (
    <section className='mt-20'>
      <div className='flex items-center justify-between mb-6'>
        <div>
          <h2 className='text-2xl font-extrabold bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent tracking-tight'>Browse Stores</h2>
          <p className='text-sm text-slate-500'>Découvrir les boutiques actives sur la plateforme</p>
        </div>
      ...
      <div className='mb-5 flex flex-col md:flex-row gap-4 md:items-center'>
        ...
        <div className='flex flex-col gap-2 md:items-end'>
          <div className='flex flex-wrap gap-2 max-w-[560px]'>
            {[{label:'Toutes les villes', value:'all'}, ...uniqueCities.map(c=>({label:c, value:c}))].map(item=>{
          ...
          <div className='flex items-center gap-3'>
            {cityFilter !== 'all' && (
              <span className='text-[11px] px-2 py-1 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200'>Filtre: {cityFilter}</span>
            )}
            <button
