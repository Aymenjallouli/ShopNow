...
export default function MyProductsManager({ shopId }){
  ...
  <div className='space-y-8'>
      <div className='flex items-center justify-between'>
        <h2 className='text-2xl font-semibold bg-gradient-to-r from-emerald-600 to-slate-800 bg-clip-text text-transparent'>Mes Produits</h2>
      </div>
  <div className='grid md:grid-cols-2 gap-8'>
        <form onSubmit={submit} className='space-y-6 bg-white/70 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-white/20'>
          ...
          <div>
            ...
            {!creatingCategory && (
              <div className='flex gap-2'>
                ...
                <button type='button' onClick={()=>{setCreatingCategory(true);}} className='px-4 py-3 rounded-xl bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition-colors'>Nouvelle</button>
              </div>
            )}
            {creatingCategory && (
                ...
                <div className='grid grid-cols-1 gap-3'>
                  <div>
                    <input 
                      ...
                      placeholder='Nom catégorie (ex: Électroménager)' 
                      value={form.newCategoryName} 
                      onChange={handleInputChange}
                      name="newCategoryName"
                    />
                    {matchedExistingCategory && (
                      <p className='mt-1 text-xs text-emerald-600'>Catégorie existante détectée: <span className='font-medium'>{matchedExistingCategory.name}</span>. Elle sera réutilisée (pas de doublon créé).</p>
                    )}
                  </div>
                  {!matchedExistingCategory && (
                  <div>
                    <label className='block text-xs font-medium text-slate-600 mb-1'>Image (Cloudinary)</label>
                    <input 
                      type='file' 
                      accept='image/*'
                      onChange={e=>{
                    ...
                    {newCategoryPreview && (
                      <div className='mt-2 flex items-center gap-3'>
                        ...
                        <button type='button' onClick={()=>{setNewCategoryImage(null); setNewCategoryPreview(null);}} className='text-xs text-red-600 hover:underline'>Retirer</button>
                      </div>
                    )}
                  </div>
                  )}
                  <div className='flex flex-wrap gap-2'>
                    <button type='button' disabled={!form.newCategoryName} onClick={createCategory} className='px-4 py-2 rounded-lg bg-emerald-600 disabled:opacity-40 text-white text-sm font-medium hover:bg-emerald-700 transition-colors flex items-center gap-1'>
                      <span>{matchedExistingCategory ? 'Utiliser' : 'Créer'}</span>
                    </button>
                    <button type='button' onClick={()=>{setCreatingCategory(false); setForm(f=>({...f,newCategoryName:''})); setNewCategoryImage(null); setNewCategoryPreview(null); setCatError(null);}} className='px-4 py-2 rounded-lg bg-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-300 transition-colors'>Annuler</button>
                  </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            {editingId && (
              <button type='button' onClick={()=>{setEditingId(null); setForm({ name:'', description:'', price:'', discount_price:'', stock:'', status:'available', image:null, category:'', featured:false, newCategoryName:'' }); setPreviewImage(null);}} className='px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-xl transition-all duration-200 border border-slate-200'>Annuler édition</button>
            )}
          </div>
        </form>
        <div className='space-y-4'>
          <h3 className='text-lg font-medium text-slate-800'>Liste</h3>
          {loading && <p>Chargement...</p>}
          {!loading && !items.length && <p className='text-sm text-slate-500'>Aucun produit.</p>}
            ...
            {items.map(p=> (
              <div key={p.id} className={`p-4 rounded-xl border ${editingId===p.id ? 'border-emerald-400 ring-2 ring-emerald-200' : 'border-slate-200'} bg-white shadow-sm flex gap-4`}>
                <div className='w-16 h-16 rounded-lg bg-slate-100 overflow-hidden flex items-center justify-center'>
                  {p.image ? <img src={p.image} alt={p.name} className='object-cover w-full h-full'/> : <span className='text-xs text-slate-400'>No Img</span>}
                </div>
                <div className='flex-1 min-w-0'>
                  <p className='font-medium text-slate-800 truncate'>{p.name}</p>
                  <p className='text-xs text-slate-500'>{p.category_name || 'Sans catégorie'}</p>
                ...
                <div className='text-xs text-slate-500 flex flex-col items-end gap-2'>
                  ...
                  <div className='flex gap-2'>
                    <button onClick={()=>startEdit(p)} className='px-2 py-1 rounded bg-slate-100 text-slate-700 hover:bg-slate-200'>Éditer</button>
                    <button onClick={()=>remove(p.id)} className='px-2 py-1 rounded bg-red-100 text-red-600 hover:bg-red-200'>Suppr</button>
                  </div>
                </div>
              </div>
            ))}
