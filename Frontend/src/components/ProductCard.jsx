...
const ProductCard = ({ product }) => {
  ...
  const { id, name = 'Product Name', price = 0, image = '', stock = 0, category, shop_id, shop_name, reviews = [], rating } = product;
    ...
    <div className="group relative">
      ...
      <div className="mt-4 space-y-1">
        <div className="flex justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-gray-500">{category?.name || 'Sans catégorie'}</p>
          </div>
        ...
        {shop_id && (
          <Link to={`/shops/${shop_id}`} className="inline-flex items-center gap-1 text-[11px] font-medium text-indigo-600 hover:text-indigo-700 group/ln">
            <span className="truncate max-w-[160px]">{shop_name || 'Voir shop'}</span>
      ...
      {reviews.length > 0 && computedRating > 0 ? (
        ...
        <div className="mt-1 min-h-[20px]">
          <span className="text-[11px] text-gray-400 italic">Pas encore d'avis</span>
        </div>
      )}
