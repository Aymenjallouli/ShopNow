...
import ShopOwnerOrders from '../orders/ShopOwnerOrders';
...
const Profile = () => {
  ...
  errors.first_name = t('profile.required_first_name');
    }
    
    if (!formData.last_name.trim()) {
  errors.last_name = t('profile.required_last_name');
    }
    
    if (!formData.username.trim()) {
  errors.username = t('profile.required_username');
    }
    
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!formData.email) {
  errors.email = t('profile.required_email');
    } else if (!emailRegex.test(formData.email)) {
  errors.email = t('profile.invalid_email');
    }
    
    // Only validate password fields if user is changing password
    if (formData.newPassword) {
      if (!formData.currentPassword) {
  errors.currentPassword = t('profile.required_current_password');
      }
      
      if (formData.newPassword.length < 8) {
  errors.newPassword = t('profile.password_length');
      }
      
      if (formData.newPassword !== formData.confirmPassword) {
  errors.confirmPassword = t('profile.passwords_no_match');
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      dispatch(updateUserProfile(updateData))
        ...
        .catch((error) => {
          // Show error message
          alert(t('profile.fail_update', { error }));
        });
    }
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <button
                    className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-all duration-200 flex items-center gap-3 ${
                      activeTab === 'shopOrders'
                        ? 'bg-gradient-to-r from-indigo-100 to-indigo-50 text-indigo-800 shadow-sm border border-indigo-200'
                        : 'text-slate-700 hover:bg-slate-50 hover:text-indigo-700'
                    }`}
                    onClick={() => handleTabChange('shopOrders')}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7l9-4 9 4-9 4-9-4zm0 6l9 4 9-4M3 7v6m18-6v6" />
                    </svg>
                    {t('profile.shop_orders')}
                  </button>
                )}
              </nav>
            </div>
          ...
          <div className="w-full lg:w-3/4">
            ...
            {activeTab === 'shopOrders' && user?.role === 'shop_owner' && (
              <div>
                <h2 className="text-2xl font-bold text-slate-800 mb-8 flex items-center gap-3">
