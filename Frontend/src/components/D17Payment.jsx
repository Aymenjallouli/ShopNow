...
const D17Payment = ({ amount, orderId, onSuccess, onError }) => {
  ...
  const initiateD17Payment = async () => {
    ...
    try {
      ...
      toast.error('Erreur lors du paiement d17');
      onError(error);
    } finally {
      setProcessing(false);
    }
    ...
    const checkStatus = async () => {
      try {
        ...
        if (data.status === 'completed') {
          ...
          toast.warning('Temps d\'attente dépassé. Veuillez vérifier votre paiement.');
      ...
      <div className="space-y-4">
        <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-4">
          <div className="flex justify-between items-center text-sm">
            <span className="text-slate-600 dark:text-slate-300">Montant à payer:</span>
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 text-center">
          <p>✓ Paiement sécurisé par d17</p>
          <p>✓ Cartes bancaires tunisiennes acceptées</p>
        </div>
      </div>
    </div>
  );
